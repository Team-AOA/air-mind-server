...
## Schedule (2022.10.10 ~ 2022.10.28)

#### Idea Brainstorming & Planning

- 2022.10.10 ~ 22.10.17
  - 아이디어 수집 및 기획
  - 구현 가능 여부 사전 조사, 기술 스택 검증
  - Mockup 작업
...
- 마인드맵 페이지 썸네일 확인 기능
- 로그인하지 않으면 Public 버전의 모든 마인드맵 확인 가능
- 로그인하지 않으면 마인드맵 수정 불가능, 노드 제목, 내용 등 확인만 가능
- 메인에서 본인이 만든 마인드맵이라면 이름 수정, 삭제 가능
...
- Socket I.O
- Styled Compontents
- Firebase Authentication (Google) (v9.10.0)
...
iframe을 이용하여 구현하였습니다. iframe의 속성인 sandbox를 `"allow-scripts allow-same-origin”`로 적용하여 같은 origin만 허용해주었고, 추가적으로 response header에 X-Frame-Options: 'SAMEORIGIN’을 적용했습니다.

처음 하고자 했던 방향은 svg의 foreignObject 엘리먼트를 이용해 마인드맵 컴포넌트를 svg 안에 포함시키고, 그것을 canvas 위에 그린 후에 canvas의 blob url을 얻어 img src로 사용하고자 했습니다. svg를 이용해 캔버스 위에 올리는 것까지는 됐지만, 스타일 적용을 위해서는 돔 트리 + css를 구문 분석하여 적용해주어야 하는 과정이 필요했는데 정해진 시간 안에 그 부분을 파악하는 것이 불가능할 것 같아 iframe을 선택하게 되었습니다.

스크린샷을 만든다는 게 쉽지 않다는 걸 느끼게 되었고 방법을 찾기 위해 라이브러리의 코드들을 많이 보게 됐는데 그 과정에서 프로젝트를 진행할 때 라이브러리 없이 혼자서 구현해보는 게 얼마나 중요한 건지 깨닫게 되었습니다.
